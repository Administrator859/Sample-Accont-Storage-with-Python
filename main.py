...
class popupWindow(object):
	...
	def __init__(self, master):
		...
		self.l = Label(top, text="Pasword: ", font=('Norwester', 14), justify=CENTER)
		self.l.pack()
	...
	def cleanup(self):
		...
		if self.value == access:
			...
			if self.attempts == 5:
				window.quit()
			self.e.delete(0, 'end')
			messagebox.showerror('Incorrect Password', 'Incorrect Password, attemps remaining: ' + str(5 - self.attempts))
...
def readfile():
	...
	for line in f:
		...
		e = entity_display(window, accountList[0], accountList[1], accountList[2], count)
		objects.append(e)
		e.display()
		count += 1
	f.close()
...
account_label = Label(window, text='Add Account', font=('Italic', 18))
...
submit = Button(window, text='Add Email', command=onsubmit, font=('Italic', 14))

account_label.grid(columnspan=3, row=0)
